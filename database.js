...
const demoData = { invoices: [], expenses: [], users: [] };
...
function getDashboardStatsDemo(companyEmail) {
  ...
  let totalExpenses = 0;
  expenses.forEach(exp => totalExpenses += exp.amount || 0);
  return {
    totalInvoiced, totalExpenses,
    netProfit: totalInvoiced - totalExpenses,
    paidInvoices, pendingInvoices,
...
async function upgradeUserToPro(userEmail) {
  try {
    if (!db) return upgradeUserToProDemo(userEmail);
    ...
    return upgradeUserToProDemo(userEmail);
  }
}

function upgradeUserToProDemo(userEmail) {
  const existingUser = demoData.users.find(u => u.email === userEmail);
  if (existingUser) {
    existingUser.plan = 'pro';
    existingUser.upgradedAt = new Date();
  } else {
    demoData.users.push({
      email: userEmail,
      plan: 'pro',
...
async function getUserPlan(userEmail) {
  try {
    if (!db) return getUserPlanDemo(userEmail);
    const doc = await db.collection('users').doc(userEmail).get();
    if (doc.exists) return doc.data();
    return { email: userEmail, plan: 'free', invoiceLimit: 5 };
  } catch (error) {
    return getUserPlanDemo(userEmail);
  }
}

function getUserPlanDemo(userEmail) {
  const user = demoData.users.find(u => u.email === userEmail);
  return user || { email: userEmail, plan: 'free', invoiceLimit: 5 };
}
